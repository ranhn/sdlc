...
关于 draft（草稿）：**当前流程不会产生该状态**。
    提交弹窗只有「提交」一个出口，VulnCreate / VulnUpdate 里都没有 status 字段，建单时
    status 直接落 pending；状态机里也没有任何动作指向 draft。因此 `submit`
    （draft -> pending）实际**不可达**，DRAFT 常量与「草稿」中文名保留，只为渲染可能存在的
    历史数据（线上实测 0 条）—— 不要再拿"草稿"去解释任何页面口径。
