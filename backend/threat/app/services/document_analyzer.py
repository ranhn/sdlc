...
6.0 **边界粒度（重要，直接影响图的可读性）**：不要把绝大多数组件塞进同一个边界 ——
   那等于没有分域：图上几乎所有流都会变成"跨边界"，虚线糊成一片、反而看不出重点
   （实测这类模型「跨边界」流占 90% 以上，"主图/全部"分层失去区分度）。
   请按组件的**实际部署位置**划分 3~5 个真实安全域，例如：用户/客户端侧、
   接入层或 DMZ、内网服务侧、数据存储侧、第三方/外部平台；
   让每个组件落在它**真正所处**的域里 —— 只有确实跨域的流才标成跨边界。
   同一个组件只属于一个边界；actor / 外部实体**不得**放进任何边界（它们在边界之外）。
